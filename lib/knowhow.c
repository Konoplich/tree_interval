...
node_t* delete_rec(ROOT, const u_int32_t key)
{
	if (key < p_root->ref.min) {
		...
		if ((key == p_root->ref.min) && (NULL == p_root->p_right)) {
			free(p_root);
			return NULL;
		}
...
void sanity_check_rec(ROOT, bool isParentRed, u_int32_t blackDepth, u_int32_t *minBlack, u_int32_t *maxBlack)
{
	...
	if (NULL != p_root->p_left) {
		...
		printf("%d %s\n",__LINE__,(p_root->p_left->ref.min < p_root->ref.min)?"ok":"assert");
	...
	if (NULL != p_root->p_right) {
		...
		printf("%d %s\n",__LINE__,(p_root->p_right->ref.min > p_root->ref.min)?"ok":"assert");
...
void traverse_rec(ROOT, u_int32_t *prev)
{
	...
	printf("%d %s\n",__LINE__,(*prev < p_root->ref.min)?"ok":"assert");
	*prev = p_root->ref.min;
	printf("%p key: %4d\n",p_root, p_root->ref.min);
